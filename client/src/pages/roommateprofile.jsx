...
export default function RoommateProfile() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        const [account, roommateProfile] = await Promise.all([
        ...
        setAccountProfile(account.user);
        if (roommateProfile) setProfile({ ...roommateProfile, location: roommateProfile.preferred_location, about: roommateProfile.bio, traits: roommateProfile.traits?.split(",").filter(Boolean) || [] });
  ...
  async function saveProfile(event) {
    ...
    const payload = { budget: Number(formData.get("budget")), preferred_location: formData.get("location"), occupation: "", lifestyle: formData.get("lifestyle"), traits: formData.getAll("traits").join(","), bio: formData.get("about") };
    try {
      const updated = await apiRequest(profile ? "/roommates/me" : "/roommates", { method: profile ? "PUT" : "POST", body: JSON.stringify(payload) });
      setProfile({ ...updated, location: updated.preferred_location, about: updated.bio, traits: updated.traits?.split(",").filter(Boolean) || [] });
      setMessage("Your roommate profile has been saved. Your matches will update when you return to Roommate Matching.");
    } catch (error) { setMessage(error.message); }
  }

  async function deleteProfile() {
    try { await apiRequest("/roommates/me", { method: "DELETE" }); navigate("/roommates"); } catch (error) { setMessage(error.message); }
  }

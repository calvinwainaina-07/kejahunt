...
export default function Bookings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const requested = propertyData?.find((property) => property.id === requestedPropertyId);
      setSelectedPropertyId((current) => current || requested?.id || propertyData?.[0]?.id || "");
